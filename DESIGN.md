...
| D-09 | Real-Chrome performance is below 100 because the Stockfish WASM must boot on `/play`. | The measured 7,295,689-byte WASM is required by the user-approved one-controller/one-board persistence contract. Route-splitting the engine would change that architecture and was declined, so 100 performance is unreachable without an unapproved architectural change. Three-sample medians are `/play` 96 mobile / 79 desktop, `/openings` 94 / 75, `/games` 98 / 77, and `/study` 98 / 77; Accessibility, Best Practices, and SEO remain 100 on every route. CI enforces performance thresholds three points below those approved medians (`/play` 93/76, `/openings` 91/72, `/games` 95/74, `/study` 95/74) to absorb ordinary audit variance without accepting a material regression. | Revisit only if the user approves changing the persistence contract or a materially smaller compatible engine becomes available; do not hide features, reset the controller, or weaken the accessibility/Best-Practices/SEO 100 floor. |
